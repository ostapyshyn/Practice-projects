...
var word = "bologna"
var lastLetter = word.removeLast()
var lastLetter2 = word.remove(at: word.index(before: word.endIndex))
//print(myWord.last)

if let myWord = myWord.last {
    print(myWord)
}
...
let combines = combineLastCharacters(words: nonsenseArray)
print(combines)
//: __Exercise 3__
//:
...
func checkNums(check: String) -> Bool {
    if Int(check) != nil {
        return true
    } else {
        return false
...
cleanArray(array: dirtyWordsArray)
...
class MovieArchive {
    ...
    func filterByDirector(dictionary: Dictionary<String,String>, name: String) -> [String] {
        
        var movies = [String]()
        

        
        for (movie, director) in dictionary {
            if name == director {
                movies.append(movie)
...
let myMovies = MovieArchive()
myMovies.filterByDirector(dictionary: movies, name: "Richard Linklater")
